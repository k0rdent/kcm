...
func checkComponentsRemoval(ctx context.Context, cl client.Client, release *kcmv1.Release, oldObj, newObj validation.ParentCluster) error {
	...
	for _, m := range removedComponents {
		...
		providers, err := validation.GetInUseProvidersWithContractsForParent(ctx, cl, prTpl, newObj)
		if err != nil {
